    ...
    def update(self):
        ...
        for virus in self.viruses:
            if not virus.doesClear():
                update_viruses.append(virus)
                try:
                    new_virus = virus.reproduce(self.getTotalPop() / self.getMaxPop())
                    update_viruses.append(new_virus)
                except NoChildException:
                    pass
        self.viruses = update_viruses
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for _ in range(numTrials):
        ...
        time_steps = []
        for t in range(300):
            viruses_remain = patient.update()
            # print(viruses_remain)
            time_steps.append(viruses_remain)
        trials.append(time_steps)
    # print(trials)
    trials = np.array(trials)
    res = np.sum(trials, axis=0) / numTrials
    T = [i for i in range(300)]
    pylab.plot(res, label="SimpleVirus")
